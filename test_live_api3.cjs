...
const loginReq = http.request({
  ...
  res.on('end', () => {
    ...
    http.request({
      ...
      path: '/api/bs/trend?period=2026-06&reporting_currency=AED&section=APPLICATION%20OF%20FUNDS',
      method: 'GET', headers: { 'Authorization': `Bearer ${token}` }
    }, res2 => {
      let data = ''; res2.on('data', d => { data += d; });
      res2.on('end', () => { console.log("TREND:", data.substring(0, 300)); });
    }).end();
    ...
    http.request({
      ...
      let data = ''; res2.on('data', d => { data += d; });
      res2.on('end', () => { console.log("SUBDIV:", data.substring(0, 300)); });
    }).end();
