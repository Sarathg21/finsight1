...
export default function LoginPage() {
  ...
  // MFA state
  const [mfaStep, setMfaStep]   = useState(false);   // show MFA input?
  const [mfaCode, setMfaCode]   = useState('');       // generated 6-digit code
  const [mfaInput, setMfaInput] = useState('');       // user's entry
  ...
  async function handleSubmit(e) {
    ...
    if (result.success) {
      // Credentials valid — show MFA. Do NOT create session yet.
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          marginLeft: 'auto',
          fontSize: '0.63rem', color: `${color}99`, fontWeight: 500,
        }}>
