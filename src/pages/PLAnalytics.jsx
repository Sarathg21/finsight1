...
export default function PLAnalytics() {
    ...
    const initial = cloneViewAllFilters(
      {
        ...
        // Preserve the main-page compare period for View All API requests.
        // This ensures the backend receives compare_period_name (e.g. Aug-26).
        comparePeriodName: appliedFilters?.comparePeriodName || '',
      },
      options
    );
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      subValue: summary?.gross_profit_pct != null ? `Margin: ${fmtPct(summary.gross_profit_pct)}` : null,
      targetText: grossProfitTarget.target != null ? fmtKPI(grossProfitTarget.target, currency) : null,
      targetVariancePct: grossProfitTarget.variancePct,
      color: '#16a34a',
      iconBg: '#f0fdf4',
      icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" fillOpacity="0.85" /></svg>,
    },
    {
      id: 'ebitda',
      label: 'EBITDA (PTD)',
  ...
  ============================================================ */}
      ...
      <div style={{ fontSize: '0.64rem', color: C.muted, display: 'flex', justifyContent: 'space-between', paddingTop: 8, flexWrap: 'wrap', gap: 4 }}>
        <span>All values in {currency} &nbsp;|&nbsp; Period: {appliedFilters.periodName || '—'} &nbsp;|&nbsp; Compared with: {appliedFilters.comparePeriodName || '—'} &nbsp;|&nbsp; As on date: {formatDataAsOf(dataAsOf) || '—'}</span>
