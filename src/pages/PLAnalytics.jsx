...
// // //   const ref = useRef(null);
// // //   useEffect(() => {
// // //     const h = (e) => { if (ref.current && !ref.current.contains(e.target)) { setOpen(false); setSearchTerm(''); } };
...
// // //   const filteredOptions = normOptions.filter(opt =>
// // //     opt.id !== 'All' &&
// // //     (!normalizedSearch || String(opt.name ?? '').toLowerCase().includes(normalizedSearch))
// // //   );

// // //   const isAll = normalizedValue.length === 0 || (normalizedValue.length === 1 && String(normalizedValue[0]) === 'All');
// // //   const selectableOptions = normOptions.filter(o => String(o.id) !== 'All');

// // //   const toggle = (optId) => {
// // //     if (String(optId) === 'All') {
// // //       // 'All' means every currently listed option is selected. Keeping the
// // //       // actual option ids in state allows the user to uncheck individual
// // //       // options while retaining the rest of the selection.
// // //       onChange(selectableOptions.map(o => o.id));
// // //       return;
// // //     }

// // //     const cur = isAll
// // //       ? selectableOptions.map(o => o.id)
// // //       : normalizedValue.filter(v => String(v) !== 'All');

// // //     const next = cur.some(v => String(v) === String(optId))
// // //       ? cur.filter(v => String(v) !== String(optId))
// // //       : [...cur, optId];

// // //     // If every listed option is selected, keep the concrete ids so any
// // //     // individual option can still be unchecked without changing the UI
// // //     // contract. Empty selection falls back to the existing All state.
// // //     onChange(next.length === 0 ? ['All'] : next);
// // //   };

// // //   const selectedVals = normOptions.filter(o => normalizedValue.some(v => String(v) === String(o.id)));
// // //   const allOptionsSelected = selectableOptions.length > 0 && selectableOptions.every(o =>
// // //     normalizedValue.some(v => String(v) === String(o.id))
// // //   );
// // //   const label = isAll || allOptionsSelected
// // //     ? placeholder
...
// // //       <div onClick={() => setOpen(o => { const next = !o; if (!next) setSearchTerm(''); return next; })} style={{ ...selStyle, backgroundImage: 'none', appearance: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', userSelect: 'none' }}>
...
// // //           <div style={{ padding: '7px 8px', borderBottom: '1px solid #f1f5f9', background: '#fff' }}>
// // //             <input
// // //               type="text"
// // //               value={searchTerm}
...
// // //               placeholder="🔍Search..."
// // //               style={{ width: '100%', boxSizing: 'border-box', padding: '6px 8px', border: '1px solid #e2e8f0', borderRadius: 6, outline: 'none', fontSize: '0.72rem', color: '#334155' }}
// // //             />
// // //           </div>

// // //           <div onClick={() => toggle('All')} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '7px 12px', cursor: 'pointer', fontSize: '0.78rem', background: (isAll || allOptionsSelected) ? '#eff6ff' : '#fff', color: (isAll || allOptionsSelected) ? '#2563eb' : '#334155', fontWeight: (isAll || allOptionsSelected) ? 600 : 400, borderBottom: '1px solid #f8fafc', whiteSpace: 'normal', lineHeight: 1.25 }} onMouseEnter={e => { if (!(isAll || allOptionsSelected)) e.currentTarget.style.background = '#f8fafc'; }} onMouseLeave={e => { if (!(isAll || allOptionsSelected)) e.currentTarget.style.background = '#fff'; }}>
// // //             <span style={{ width: 14, height: 14, border: '1.5px solid ' + ((isAll || allOptionsSelected) ? '#2563eb' : '#cbd5e1'), borderRadius: 3, background: (isAll || allOptionsSelected) ? '#2563eb' : '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
// // //               {(isAll || allOptionsSelected) && <span style={{ color: '#fff', fontSize: '0.6rem', lineHeight: 1 }}>✓</span>}
// // //             </span>
// // //             All
// // //           </div>
...
// // //         <MultiSelect
// // //           options={filterOptions?.legalEntities || ['All']}
// // //           value={filters?.legalEntityId || ['All']}
...
// // //         <MultiSelect
// // //           options={filterOptions?.parentDivisions || ['All']}
// // //           value={filters?.parentDivisionId || ['All']}
...
// // //         <MultiSelect
// // //           options={filterOptions?.subdivisions || ['All']}
// // //           value={filters?.subdivisionId || ['All']}
...
// // //     minimumFractionDigits: Math.abs(n) >= 1_000_000 ? 2 : 0,
...
// // //   const label = String(row?.particulars ?? '—');
...
// // //   /* Backend PTD/YTD values. Target PTD is the target returned for the
// // //      currently selected filters, including the selected Compare With month. */
// // //   const actualPTD = row?.current_ptd;
// // //   const comparePTD = row?.compare_ptd;
// // //   const targetPTD = row?.target_ptd;
// // //   const actualYTD = row?.current_ytd;
// // //   const targetYTD = row?.target_ytd;

// // //   const backendVarActualVsCompare = row?.variance_pct_current_vs_compare;
// // //   const backendVarActualVsTarget = row?.variance_pct_current_vs_target;
// // //   const backendVarYTDVsTarget = row?.variance_pct_ytd_vs_target;
...
// // //   // IMPORTANT: variance values are display-only backend values.
// // //   // Do not recalculate, reverse, or invert them in the frontend.
// // //   const varianceActualVsCompare = toNumber(backendVarActualVsCompare);
// // //   const varianceActualVsTarget = toNumber(backendVarActualVsTarget);
// // //   const varianceYTDVsTarget = toNumber(backendVarYTDVsTarget);
...
// // //   // CFO UAT2: backend owns the expense/income favourability rule.
...
// // //     <tr
// // //       style={{
// // //         background: hover && !isSubtotal ? '#f0f6ff' : rowBg,
// // //         transition: 'background 0.1s',
// // //       }}
// // //       onMouseEnter={() => {
...
// // //           fontWeight: isImportantSubtotal || isSubtotal ? 700 : 400,
// // //           color: labelColor,
// // //           fontSize: isImportantSubtotal ? '0.76rem' : '0.74rem',
// // //           borderBottom: `1px solid ${isSubtotal ? '#e2e8f0' : '#f1f5f9'}`,
// // //         }}
// // //       >
// // //         {label}
// // //       </td>
...
// // //           ...TDx,
// // //           fontWeight: isImportantSubtotal || isSubtotal ? 700 : 400,
// // //           color: amountColor(actualPTD),
// // //         }}
// // //       >
// // //         {formatAmount(actualPTD)}
// // //       </td>

// // //       <td
// // //         style={{
// // //           ...TDx,
// // //           color: amountColor(comparePTD, C.slate),
// // //         }}
// // //       >
...
// // //         style={{
// // //           ...TDx,
// // //           color: amountColor(targetPTD, C.slate),
// // //         }}
// // //       >
...
// // //         style={{
// // //           ...TDx,
// // //           color: favourability.color,
// // //           fontWeight: varianceActualVsCompare == null ? 400 : 600,
// // //         }}
// // //       >
// // //         {formatPercent(varianceActualVsCompare)}
// // //       </td>

// // //       <td
// // //         style={{
// // //           ...TDx,
// // //           color: varianceColor(varianceActualVsTarget),
// // //           fontWeight: varianceActualVsTarget == null ? 400 : 600,
// // //         }}
// // //       >
// // //         {formatPercent(varianceActualVsTarget)}
// // //       </td>
...
// // //           ...TDx,
// // //           fontWeight: isImportantSubtotal || isSubtotal ? 700 : 400,
// // //           color: amountColor(actualYTD),
// // //         }}
// // //       >
// // //         {formatAmount(actualYTD)}
// // //       </td>

// // //       <td
// // //         style={{
// // //           ...TDx,
// // //           color: amountColor(targetYTD, C.slate),
// // //         }}
// // //       >
...
// // //         style={{
// // //           ...TDx,
// // //           color: varianceColor(varianceYTDVsTarget),
// // //           fontWeight: varianceYTDVsTarget == null ? 400 : 600,
// // //         }}
// // //       >
// // //         {formatPercent(varianceYTDVsTarget)}
// // //       </td>
// // //     </tr>
// // //   );
// // // }
...
// // //   const [secIncome, setSecIncome] = useState(true);
// // //   const [secCOGS, setSecCOGS] = useState(true);
// // //   const [secGrossProfit, setSecGrossProfit] = useState(true);
// // //   const [secProfitability, setSecProfitability] = useState(true);
// // //   const [secExpenses, setSecExpenses] = useState(true);
// // //   const [secOtherExpenses, setSecOtherExpenses] = useState(true);

// // //   // Keep the View All hierarchy aligned with the requested P&L statement style:
// // //   // Gross Profit is shown separately, while EBITDA/PBT/Tax/Net Profit remain
// // //   // under the Profitability section. The underlying API data is unchanged.
// // //   const grossProfitRows = profitabilityRows.filter((row) => {
...
// // //   const remainingProfitabilityRows = profitabilityRows.filter((row) => {
// // //     const label = String(row?.particulars ?? row?.label ?? row?.name ?? '')
// // //       .trim()
// // //       .toLowerCase();
// // //     return label !== 'gross profit';
...
// // //   // Sticky header and horizontal-scroll synchronization for the View All statement.
// // //   const statementHeaderRef = useRef(null);
// // //   const statementBodyScrollRef = useRef(null);
...
// // //                     }}
// // //                   >
// // //                     {comparePeriodName || 'Compare Month'}
// // //                   </span>
// // //                 </th>
...
// // //             {/* <SectionHeader
// // //             label="GROSS PROFIT"
// // //             expanded={secGrossProfit}
// // //             onToggle={() => setSecGrossProfit((p) => !p)}
// // //             colSpan={9}
// // //           /> */}

// // //             {grossProfitRows.map((row, i) => {
// // //               if (!secGrossProfit) return null;

// // //               return (
// // //                 <PLRow
// // //                   key={`gross-profit-${row.display_order ?? i}`}
// // //                   row={row}
// // //                   indent
// // //                   currency={currency}
// // //                   tdStyles={MTD}
...
// // //               onToggle={() => setSecOtherExpenses((p) => !p)}
// // //               colSpan={9}
// // //             />
...
// // //             {/* =========================
// // //               PROFITABILITY
// // //               EBITDA / PBT / Tax / Net Profit
// // //           ========================= */}

// // //             <SectionHeader
// // //               label="PROFITABILITY"
// // //               expanded={secProfitability}
// // //               onToggle={() => setSecProfitability((p) => !p)}
// // //               colSpan={9}
// // //             />

// // //             {remainingProfitabilityRows.map((row, i) => {
// // //               const visible =
...
// // //     exportPL(section, format, viewAllFilters)
// // //       .then(() => {
// // //         showToast(
// // //           `${format.toUpperCase()} export downloaded successfully.`,
// // //           'success'
...
// // //         ? `Margin: ${fmtPct(summary.gross_profit_pct)}`
// // //         : null,
...
// // //       subValue: summary?.ebitda_pct != null ? `Margin: ${fmtPct(summary.ebitda_pct)}` : null,
...
// // //       value: fmtPct(summary?.net_profit_pct),
// // //       targetText: netProfitMarginTarget.target != null ? fmtPct(netProfitMarginTarget.target) : null,
...
// // //       value: fmtPct(summary?.ebitda_pct),
// // //       targetText: ebitdaMarginTarget.target != null ? fmtPct(ebitdaMarginTarget.target) : null,
...
// // //   // Gross Profit is supplied by the backend under the PROFITABILITY section,
// // //   // but it is displayed as a standalone subtotal immediately after Cost of Sales.
// // //   // Keep the original API data untouched and only split the display rows here.
// // //   const grossProfitRows = profitabilityRows.filter((row) => {
// // //     const label = String(row?.particulars ?? row?.label ?? row?.name ?? '')
// // //       .trim()
// // //       .toLowerCase();
...
// // //                   {/* ============================================================
// // //       GROSS PROFIT
// // //       Supplied by the existing PROFITABILITY API section, but displayed here
// // //       as the subtotal immediately after COST OF SALES.
// // //   ============================================================ */}
// // //                   {grossProfitRows.map((row, i) => (
// // //                     <PLRow
// // //                       key={`gross-profit-${row.display_order ?? i}`}
...
// // //       PROFITABILITY
// // //       EBITDA / PBT / Tax / Net Profit
// // //       stay visible when collapsed because they are subtotals.
// // //   ============================================================ */}
// // //                   <SectionHeader
// // //                     label="PROFITABILITY"
// // //                     expanded={secProfitability}
...
// //    * IMPORTANT:
// //    * - []       = nothing selected
// //    * - ['All']  = All/default filter state
// //    * - [ids...] = actual selected values
// //    *
// //    * This distinction lets Clear show every option as unchecked while
// //    * keeping the closed filter display as "All".
// //    */
// //   const hasExplicitAll = normalizedValue.some(
// //     (item) => String(item) === 'All'
// //   );

// //   const selectedConcreteValues = normalizedValue.filter(
// //     (item) => String(item) !== 'All'
// //   );
...
// //     hasExplicitAll || allOptionsSelected;

// //   const toggle = (optionId) => {
// //     /* -----------------------------------------------
// //        SELECT ALL
// //     ----------------------------------------------- */
// //     if (String(optionId) === 'All') {
// //       onChange(selectableOptions.map((option) => option.id));
// //       return;
// //     }
...
// //        INDIVIDUAL VALUE
// //     ----------------------------------------------- */
// //     const currentValues = hasExplicitAll
// //       ? selectableOptions.map((option) => option.id)
// //       : selectedConcreteValues;

// //     const exists = currentValues.some(
// //       (selected) => String(selected) === String(optionId)
// //     );
...
// //   const clearAll = () => {
// //     /*
// //      * Empty array means no values are selected.
// //      * The closed filter still displays "All" because there is no
// //      * active restriction.
// //      */
// //     onChange([]);
// //     setSearchTerm('');
...
// //   const selectedVals = normOptions.filter((option) =>
// //     selectedConcreteValues.some(
// //       (selected) => String(selected) === String(option.id)
// //     )
// //   );
...
// //   const label =
// //     isNoneSelected || hasExplicitAll || allOptionsSelected
// //       ? placeholder
// //       : selectedVals.length === 1
// //         ? selectedVals[0].name
// //         : `${selectedVals.length} selected`;
...
// //               cursor: 'pointer',
// //               fontSize: '0.78rem',
// //               background: allRowSelected ? '#eff6ff' : '#fff',
// //               color: allRowSelected ? '#2563eb' : '#334155',
// //               fontWeight: allRowSelected ? 600 : 400,
...
// //                 width: 14,
// //                 height: 14,
// //                 border: `1.5px solid ${allRowSelected ? '#2563eb' : '#cbd5e1'}`,
// //                 borderRadius: 3,
// //                 background: allRowSelected ? '#2563eb' : '#fff',
// //                 display: 'inline-flex',
// //                 alignItems: 'center',
// //                 justifyContent: 'center',
// //                 flexShrink: 0,
...
// //               const selected = selectedConcreteValues.some(
// //                 (selectedValue) =>
// //                   String(selectedValue) === String(option.id)
// //               );
...
//   /*
//    * IMPORTANT:
//    * - []       = no specific values selected -> closed dropdown displays "All"
//    * - [ids...] = actual selected values
//    *
//    * "All" is NOT a selectable checkbox/value. The Select All button selects
//    * all concrete options, while Clear returns the value to an empty array.
//    */
//   const normalizedValue = Array.isArray(value)
...
//   // Never render a backend/frontend "All" entry as an individual option.
//   const selectableOptions = normOptions.filter(
//     (option) =>
//       option?.id !== null &&
//       option?.id !== undefined &&
//       String(option.id) !== '' &&
//       String(option.id) !== 'All'
//   );
...
//   const toggle = (optionId) => {
//     const exists = selectedConcreteValues.some(
//       (selected) => String(selected) === String(optionId)
//     );

//     const nextValues = exists
//       ? selectedConcreteValues.filter(
//         (selected) => String(selected) !== String(optionId)
//       )
...
//   const label =
//     selectedConcreteValues.length === 0 || allOptionsSelected
//       ? placeholder
//       : selectedVals.length === 1
//         ? selectedVals[0].name
//         : `${selectedVals.length} selected`;
...
//           {/* Select All / Clear - there is intentionally NO "All" checkbox row */}
//           <div
//             style={{
//               display: 'flex',
//               justifyContent: 'space-between',
