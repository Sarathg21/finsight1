...
const MOCK_FILTERS = {
  legal_groups: ['FJ Group', 'RKME Group'],

  legal_entities: [
    'Alpha Ducts LLC',
    'Alpine Coils Industry LLC',
    'DC Serve Equipment Trading LLC',
...
function getMockDataForPath(path, params = {}) {
  ...
  if (path.includes('/trend')) {
    if (params.period_name) {
      const idx = MOCK_TREND.findIndex(
        m => m.period_name === params.period_name
      );

      if (idx !== -1) {
        // Selected month + previous 5 months
        return MOCK_TREND.slice(
          Math.max(0, idx - 5),
          idx + 1
        );
      }
    ...
    // Default: latest 6 months
    return MOCK_TREND.slice(-6);
  }
...
export async function fetchPLFilters(filters = {}) {
  ...
  const normalizeIdName = (list) => {
      ...
      if (typeof e === 'object' && e !== null) {
        return {
          id: String(
            e.value !== undefined
              ? e.value
              : (e.id !== undefined ? e.id : String(i))
          ),
          name: String(
            e.label !== undefined
              ? e.label
              : (e.name !== undefined ? e.name : String(e))
          ),
        };
      }
    ...
    // Remove backend dummy "All" option
  ...
  return {
    ...
    years: Array.isArray(res.years)
      ? res.years
      : [],

    periods: Array.isArray(res.periods)
      ? res.periods
      : [],

    comparePeriods: Array.isArray(res.compare_periods)
      ? res.compare_periods
      : [],

    currencies: Array.isArray(res.currencies) && res.currencies.length > 0
      ? res.currencies
      : ['AED'],
  };
}
...
export async function fetchPLComparison(filters = {}) {
  try {
    // IMPORTANT:
    // apiCall() expects the query params object directly.
    // Do NOT wrap it inside { params: ... }.
    ...
    return rows.map((item) => {
      ...
      return {
        ...
        // IMPORTANT:
        // Use the backend favourability directly.
        // Do NOT calculate this from variance.
        favourability:
          item.favourability ??
          item.Favourability ??
          'neutral',
    ...
    console.error('fetchPLComparison failed:', error);
    throw error;
  }
}
...
/**
 * GET /api/pl/statement
 * Full P&L Detailed Statement table rows.
 *
 ...
 *     compare_ptd,
 *     target_ptd,
 *     variance_pct_current_vs_compare,
 *     variance_pct_current_vs_target,
 *     current_ytd,
 *     target_ytd,
 *     variance_pct_ytd_vs_target
 *   }
 * ]
 *
 * Target-related fields may currently be null.
 * IMPORTANT:
 * Never convert null target values to 0.
 */
export async function fetchPLStatement(filters = {}) {
  ...
  if (
    ...
    !Array.isArray(res) &&
    (
      Array.isArray(res.income) ||
      Array.isArray(res.cost_of_sales) ||
      Array.isArray(res.expenses) ||
      Array.isArray(res.bottom)
    )
  ) {
    return res;
  }
  ...
  // ------------------------------------------------------------
  // Extract the backend flat array
  //
  // apiCall() wraps a direct backend array as:
  // { data: [...] }
  // ------------------------------------------------------------
  const rawRows =
    Array.isArray(res)
  ...
  const rows = rawRows.map((item, index) => {
    const normalized = {
      ...
      // ----------------------------------------------------------
      variance_pct_current_vs_compare:
        item.variance_pct_current_vs_compare ??
        null,

      variance_pct_current_vs_target:
        item.variance_pct_current_vs_target ??
        null,
      ...
      //
      // IMPORTANT:
      // Actual backend field = variance_pct_ytd_vs_target
      // ----------------------------------------------------------
      variance_pct_ytd_vs_target:
        item.variance_pct_ytd_vs_target ??
        null,

      // ----------------------------------------------------------
      // CFO hierarchy metadata
      // ----------------------------------------------------------
      row_type:
        item.row_type ??
        item.rowType ??
      ...
      // ----------------------------------------------------------
      // Preserve ALL original backend fields
      // ----------------------------------------------------------
      ...item,
    };

    // ------------------------------------------------------------
    // Re-apply normalized fields after ...item
    // so component always receives these exact names.
    // ------------------------------------------------------------
    return {
      ...normalized,
  ...
  // ------------------------------------------------------------
  // Backend controls statement order.
  // ------------------------------------------------------------
  rows.sort(
    (a, b) =>
  ...
  console.log(
    '[fetchPLStatement] normalized rows:',
    rows
  );
